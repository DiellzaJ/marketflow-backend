...
public static class ServiceCollectionExtensions
{
        ...
        IConfiguration configuration)
    {
        ...
        services.AddCors(options =>
        {
            ...
            {
                policy
                    .WithOrigins(GetFrontendUrls(configuration))
                    .AllowAnyHeader()
                    .AllowAnyMethod();
            });
        });
    ...
    private static string CreatePostgresConnectionString(IConfiguration configuration)
    {
        ...
        var portValue = GetConfigurationValue(configuration, "DB_PORT", "Database:Port", "5432");
        ...
        {
            Host = GetConfigurationValue(configuration, "DB_HOST", "Database:Host", "localhost"),
            Port = port,
            Database = GetConfigurationValue(configuration, "DB_NAME", "Database:Name", "marketflow_db"),
            Username = GetConfigurationValue(configuration, "DB_USERNAME", "Database:Username", "postgres"),
            Password = GetConfigurationValue(configuration, "DB_PASSWORD", "Database:Password", string.Empty)
        }.ConnectionString;
    }
    ...
    {
        return (configuration["Cors:FrontendUrl"] ?? "http://localhost:5173")
